    ...
    <nav
        ...
        <div class="md:col-span-3">
            <a class="flex-none rounded-xl text-xl inline-block font-semibold focus:outline-none focus:opacity-80"
                href={{ route('home') }}>
                <img class="w-28 h-auto" src="{{ asset('assets/images/general/logo.png') }}" alt="Logo Image">
            </a>
        </div>
        ...
        <div class="flex items-center gap-x-1 md:gap-x-2 ms-auto py-1 md:ps-6 md:order-3 md:col-span-3">
            ...
            @auth
                <form method="post" action="{{ route('auth.logout') }}">
                    ...
                    <x-forms.button type="submit" text="logout" class="py-2 px-3" />
                </form>
            @endauth
        ...
        <div id="hs-navbar-hcail"
            ...
            <div
                ...
                @foreach ($items as $item)
                    <div>
                        <a href="{{ $item['url'] }}"
                            class="relative inline-block text-black focus:outline-none before:absolute before:bottom-0.5 before:start-0 before:-z-[1] before:w-full before:h-1 {{ request()->routeIs($item['active']) ? 'border-b-2 border-lime-400' : '' }}">
                            {{ $item['label'] }}
                        </a>
                    </div>
                @endforeach
